...
void InsercaoInicial(Filmes **inicioF, Filmes **fimF, Atores **inicioA, Atores **fimA, Diretores **inicioD, Personagens **inicioP, char ValorASerInserido[2100]); // Função que define quais outras funçÃµes sãoo chamadas, partindo do valor concatenado que recebe.
		...
		if(ValorASerInserido[i]==',' || ValorASerInserido[i+1]=='\0')
		{
			...
			else if (status==2) //diretor
			{
				...
				if(ValorASerInserido[m]=='d' && ValorASerInserido[m+1]=='i' && ValorASerInserido[m+2]=='r' && ValorASerInserido[m+3]==':')
				{
					...
					{
						l=0;
						while(ValorASerInserido[m]!=',' && ValorASerInserido[m]!='\0') // <--- Nãoo encontrei caso que fosse necessário (programa passou a ter comportamento estranho)
						{
						...
						else
						{
							...
							i = m; // ATENÃ‡ãoO: variável i é incrementada aqui pois outro nome de diretor fora inserido
			...
			else if(status>2 && aux[0]!='\0')
			{
				...
				if(flagPersonagem==1) // Se possuir um =, existe o personagem na string aux.
				{
					n = 0; // zera n;
					o++;
					while(aux[o]!='\0') // Enquanto nãoo estiver no fim da string auxiliar, continua a inserção.
				 	{
				...
				novoA = verificaAtores(inicioA,nomeAtor); // Aqui é verificada a existÃªncia do determinado ator. Se existe, retorna a posição da struct;
...
void menuInserirFilme(Filmes **inicioF, Filmes **fimF, Atores **inicioA, Atores **fimA, Diretores **inicioD, Personagens **inicioP)
{
	...
	system("cls");
		...
		else //... Se nãoo existe, concatena na string que será passada como parÃ¢metro para a função de Inserção.
		{
		...
		if(toupper(simNao)=='S')
		{
			...
			strcat(ValorASerInserido,compAno); // Concatena o ano e uma vÃ­rgula
		}
		strcat(ValorASerInserido, ",");
		fflush(stdin);
		...
		if(toupper(simNao)=='S')
		{
			...
			printf("\n Existe(m) outro(s) Diretor(es) que participou(participaram) do filme?(S/N)\n"); // Se existirem outros diretores, concatenará uma vÃ­rgula e irá ler o nome dos diretores, um por um.
			simNao = getch();
...
{												// Com scanf ou gets nãoo reconheceria, em virtude da tabela ASCII.
	...
	if(*inicioAR==NULL){//em caso de nao ter nenhum ator relacionado cadastrado
		...
		while(ptauxAR!=NULL){
			...
			{								//o ator ja existe
				...
				while(ptauxAR->proxF!=NULL){ //percorre até o ultimo filme do ator
	...
	if(*inicioAD==NULL){//em caso de nao ter nenhum ator relacionado cadastrado
		...
		while(ptauxAD!=NULL){
			if(strcmp((ptauxAD)->Nome, ator)==0){//o ator ja existe
				while(ptauxAD->proxF!=NULL){ //percorre atÉ o ultimo filme do ator
...
void ListarAtoresRelacionadosAtores(Atores **inicioA){
	...
	FilmesAtores *ptauxFA;
	AtoresFilmes *ptauxAF;
	AtoresRelacionados *inicioAR, *headAR;//head é o cabecalho	
	...
	while(toupper(flag)=='S')
	{
		...
		ptauxFA = ptauxA->filmes;
		ptauxAF;
		inicioAR=NULL, headAR=NULL;//head é o cabecalho	
		...
		if(ptauxA!=NULL)// verifica se ator existe
		{
			...
			while(ptauxFA!=NULL)
			{
				...
				while(ptauxAF->prox!= NULL){  //verifica se o ator é o unico listado no filme
					if(strcmp(ptauxAF->nome->NomeAtor,ptauxA->NomeAtor)!=0){//verifica se o ator nao é o selecionado
			...
			ptauxAR = headAR = inicioAR;// Recebe a primeira posição
			int flag;
			while(ptauxAR!=NULL)
			{
				printf("\nAtor: %s: ", ptauxAR->Nome);
...
struct nodoFilmes *verificaFilmes(Filmes **inicioF, char nome[100]) // verificação se existe filme. Se existe, retorna o filme.
{
	...
	while(ptaux!=NULL)
	{
		...
		{
			return ptaux;			// Se encontra o filme, retorna ele, senãoo...
		}
		ptaux = ptaux->prox;
	}
...
void InsereAtorListaAFilmes(AtoresFilmes **inicioAF, Atores **Pos ) // Insere no inÃ­cio da lista AtoresFilmes na lista Filmes * favor verificar em inserçãoInicial *
{
...
/* Sessãoo de funções relacionadas aos personagens; Mesmo padrão que os acima */
